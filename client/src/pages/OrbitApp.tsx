...
import { Bell, CalendarDays, Camera, Check, ChevronRight, CircleOff, Compass, Copy, Heart, ImagePlus, Layers, Loader2, LockKeyhole, LogOut, MapPin, Menu, MessageCircleHeart, MoonStar, Plus, Quote, Radio, RotateCw, Search, Settings, ShieldCheck, Sparkles, Star, X } from "lucide-react";
...
const ACCEPTED_MOMENT_MIME_TYPES = ["image/jpeg", "image/png", "image/webp", "image/gif", "image/avif", "image/heic", "image/heif", "video/mp4", "video/webm", "video/quicktime", "video/x-m4v"] as const;
type MomentMimeType = typeof ACCEPTED_MOMENT_MIME_TYPES[number];
const MIME_BY_EXTENSION: Record<string, MomentMimeType> = { jpg: "image/jpeg", jpeg: "image/jpeg", png: "image/png", webp: "image/webp", gif: "image/gif", avif: "image/avif", heic: "image/heic", heif: "image/heif", mp4: "video/mp4", webm: "video/webm", mov: "video/quicktime", m4v: "video/x-m4v" };
function detectMomentMimeType(file: File): MomentMimeType | null {
  if ((ACCEPTED_MOMENT_MIME_TYPES as readonly string[]).includes(file.type)) return file.type as MomentMimeType;
...
function PrivateGate({ children }: { children: React.ReactNode }) {
  ...
  if (!isAuthenticated) {
    return <main className="welcome-shell"><div className="ambient-orb orb-one" /><div className="ambient-orb orb-two" />
      <div className="welcome-card"><div className="brand-mark"><span /> <b>OUR ORBIT</b></div><div className="eyebrow"><LockKeyhole size={14} /> PRIVATE FOR US</div>
...
function HomePage() {
  ...
  const sharedPhotos = moments.data?.filter(({ moment }) => moment.mediaType === "photo" && moment.visibility === "pair") ?? [];
  const cover = sharedPhotos.find(({ moment }) => moment.id === selectedCoverId)?.moment.mediaUrl ?? sharedPhotos[0]?.moment.mediaUrl;
...
function MomentsPage() {
  ...
  const relationship = trpc.orbit.relationship.get.useQuery();
  const updateFeaturedRotation = trpc.orbit.relationshipExtras.updateFeaturedRotation.useMutation();
  const rotateFeatured = trpc.orbit.relationshipExtras.rotateFeatured.useMutation();
  const setFeatured = trpc.orbit.relationshipExtras.setFeatured.useMutation();
  const albums = trpc.orbit.albums.list.useQuery();
  const createAlbum = trpc.orbit.albums.create.useMutation();
  const removeAlbum = trpc.orbit.albums.remove.useMutation();
  const createMilestone = trpc.orbit.albums.milestones.create.useMutation();
  const removeMilestone = trpc.orbit.albums.milestones.remove.useMutation();
  const [file, setFile] = useState<File | null>(null); const [caption, setCaption] = useState(""); const [quote, setQuote] = useState(""); const [songTitle, setSongTitle] = useState(""); const [songArtist, setSongArtist] = useState(""); const [songUrl, setSongUrl] = useState(""); const [date, setDate] = useState(new Date().toISOString().slice(0, 10)); const [filter, setFilter] = useState<"all" | "photo" | "video">("all"); const [query, setQuery] = useState(""); const [favoritesOnly, setFavoritesOnly] = useState(false); const [visibility, setVisibility] = useState<"pair" | "private">("pair"); const [albumId, setAlbumId] = useState<number | null>(null); const [activeAlbumId, setActiveAlbumId] = useState<number | null>(null); const [albumName, setAlbumName] = useState(""); const [albumDescription, setAlbumDescription] = useState(""); const [albumStartDate, setAlbumStartDate] = useState(""); const [albumEndDate, setAlbumEndDate] = useState(""); const [albumCoverMomentId, setAlbumCoverMomentId] = useState(""); const [milestoneTitle, setMilestoneTitle] = useState(""); const [milestoneNote, setMilestoneNote] = useState(""); const [milestoneDate, setMilestoneDate] = useState(new Date().toISOString().slice(0, 10)); const [featuredRotationEnabled, setFeaturedRotationEnabled] = useState(false); const [featuredRotationMode, setFeaturedRotationMode] = useState<"manual" | "weekly" | "monthly" | "anniversary">("manual"); const [editingMoment, setEditingMoment] = useState<{ id: number; caption: string; quote: string; songTitle: string; songArtist: string; songUrl: string; occurredAt: string } | null>(null); const inputRef = useRef<HTMLInputElement>(null); const cameraInputRef = useRef<HTMLInputElement>(null);
  const milestones = trpc.orbit.albums.milestones.list.useQuery({ albumId: activeAlbumId ?? 0 }, { enabled: Boolean(activeAlbumId) });
  const visibleMoments = moments.data?.filter(({ moment }) => (filter === "all" || moment.mediaType === filter) && (!favoritesOnly || moment.favorite) && (!query.trim() || `${moment.caption ?? ""} ${moment.quote ?? ""} ${formatDate(moment.occurredAt)}`.toLowerCase().includes(query.trim().toLowerCase()))) ?? [];
  const featuredRelation = relationship.data?.relationship;
  const featuredMoment = moments.data?.find(({ moment }) => moment.id === featuredRelation?.featuredMomentId && moment.mediaType === "photo" && moment.visibility === "pair")?.moment;
  useEffect(() => { if (!featuredRelation) return; setFeaturedRotationEnabled(Boolean(featuredRelation.featuredRotationEnabled)); setFeaturedRotationMode(featuredRelation.featuredRotationMode); }, [featuredRelation?.featuredRotationEnabled, featuredRelation?.featuredRotationMode]);
  useEffect(() => { if (!featuredRelation?.featuredRotationEnabled || featuredRelation.featuredRotationMode === "manual" || !featuredRelation.featuredRotatedAt) return; const elapsed = Date.now() - new Date(featuredRelation.featuredRotatedAt).getTime(); const interval = featuredRelation.featuredRotationMode === "weekly" ? 7 * 86_400_000 : featuredRelation.featuredRotationMode === "monthly" ? 30 * 86_400_000 : 365 * 86_400_000; if (elapsed >= interval) rotateFeatured.mutateAsync().then(() => relationship.refetch()).catch(() => undefined); }, [featuredRelation?.featuredRotationEnabled, featuredRelation?.featuredRotationMode, featuredRelation?.featuredRotatedAt]);
  const saveFeaturedRotation = async () => { await updateFeaturedRotation.mutateAsync({ enabled: featuredRotationEnabled, mode: featuredRotationMode }); await relationship.refetch(); toast.success("Featured-memory rotation preferences saved."); };
  const rotateFeaturedNow = async () => { try { await rotateFeatured.mutateAsync(); await relationship.refetch(); toast.success("The featured memory changed."); } catch (error) { toast.error(error instanceof Error ? error.message : "Add a shared photo before rotating featured memories."); } };
  const upload = async (event: React.FormEvent) => {
    ...
    const mimeType = detectMomentMimeType(file);
    if (!mimeType) return toast.error("Choose a supported photo or video format.");
    try {
      ...
      await create.mutateAsync({ filename: file.name, mimeType, fileKey: prepared.key, mediaUrl: prepared.url, caption: caption || undefined, quote: quote || undefined, albumId: albumId ?? undefined, songTitle: songTitle || undefined, songArtist: songArtist || undefined, songUrl: songUrl || undefined, visibility, fileSizeBytes: file.size, occurredAt: new Date(`${date}T12:00:00`) });
  ...
  if (moments.error) return <DataError title="Your private moments could not be loaded." />;
    return <div className="page-grid"><GlassCard className="featured-memory-card"><div className="panel-heading"><div><div className="eyebrow"><Sparkles size={14} /> FEATURED MEMORY</div><h3>{featuredMoment?.caption || "Choose a memory to feature"}</h3></div><Heart size={19} /></div>{featuredMoment ? <div className="featured-memory-content"><img src={featuredMoment.mediaUrl} alt={featuredMoment.caption || "Featured shared memory"} /><div><small>{formatDate(featuredMoment.occurredAt)} · visible to both of us</small>{featuredMoment.quote && <blockquote className="moment-quote"><Quote size={14} />{featuredMoment.quote}</blockquote>}</div></div> : <div className="empty-inline"><ImagePlus size={20} /><span>Feature a shared photo so this gallery has a memory to return to.</span></div>}<div className="featured-memory-controls"><label className="switch-row"><span>Rotate featured memory</span><input type="checkbox" checked={featuredRotationEnabled} onChange={(event) => setFeaturedRotationEnabled(event.target.checked)} /><i /></label><select value={featuredRotationMode} onChange={(event) => setFeaturedRotationMode(event.target.value as typeof featuredRotationMode)}><option value="manual">Manual</option><option value="weekly">When opened weekly</option><option value="monthly">When opened monthly</option><option value="anniversary">On anniversary month</option></select><div className="record-actions"><button className="secondary-button" onClick={saveFeaturedRotation} disabled={updateFeaturedRotation.isPending}>Save rotation</button><button className="primary-button" onClick={rotateFeaturedNow} disabled={rotateFeatured.isPending}><RotateCw size={15} /> Rotate now</button></div></div></GlassCard><GlassCard className="gallery-albums-card"><div className="panel-heading"><div><div className="eyebrow"><Layers size={14} /> PRIVATE GALLERY ALBUMS</div><h3>Give memories a chapter</h3></div><span>{albums.data?.length ?? 0} albums</span></div><p>Albums and their milestones stay inside this linked orbit. Both of us can add shared photos and mark the moments that shaped each chapter.</p><form className="album-create-form" onSubmit={async (event) => { event.preventDefault(); if (!albumName.trim()) return; const created = await createAlbum.mutateAsync({ name: albumName, description: albumDescription || undefined, startedAt: albumStartDate ? new Date(`${albumStartDate}T12:00:00`) : undefined, endedAt: albumEndDate ? new Date(`${albumEndDate}T12:00:00`) : undefined, coverMomentId: albumCoverMomentId ? Number(albumCoverMomentId) : undefined }); await utils.orbit.albums.list.invalidate(); setAlbumName(""); setAlbumDescription(""); setAlbumStartDate(""); setAlbumEndDate(""); setAlbumCoverMomentId(""); if (created.id) setActiveAlbumId(created.id); toast.success("Private album created."); }}><label>Album name<input value={albumName} onChange={(event) => setAlbumName(event.target.value)} maxLength={120} placeholder="Summer together" required /></label><label>Description <span>(optional)</span><input value={albumDescription} onChange={(event) => setAlbumDescription(event.target.value)} maxLength={500} placeholder="The little chapter behind these photos" /></label><button className="secondary-button" disabled={createAlbum.isPending}>{createAlbum.isPending ? <Loader2 className="spin" size={15} /> : "Create album"}<Plus size={15} /></button></form><div className="album-list">{albums.data?.length ? albums.data.map(({ album, momentCount, milestoneCount }) => <div className={`album-card ${activeAlbumId === album.id ? "active" : ""}`} key={album.id}><button type="button" className="album-select" onClick={() => setActiveAlbumId(album.id)}><span><b>{album.name}</b><small>{momentCount} memories · {milestoneCount} milestones{album.description ? ` · ${album.description}` : ""}</small></span><Layers size={16} /></button>{album.createdById === user?.id && <button className="quiet-delete" onClick={async () => { await removeAlbum.mutateAsync({ id: album.id }); await utils.orbit.albums.list.invalidate(); if (activeAlbumId === album.id) setActiveAlbumId(null); toast.success("Album removed; its memories stayed safely in the gallery."); }}>Remove</button>}</div>) : <div className="empty-inline"><Layers size={20} /><span>Create the first album for a chapter of your story.</span></div>}</div>{activeAlbumId && <div className="album-milestones"><div className="eyebrow"><Star size={13} /> ALBUM MILESTONES</div><form className="milestone-create-form" onSubmit={async (event) => { event.preventDefault(); if (!milestoneTitle.trim()) return; await createMilestone.mutateAsync({ albumId: activeAlbumId, title: milestoneTitle, note: milestoneNote || undefined, milestoneDate: new Date(`${milestoneDate}T12:00:00`) }); await utils.orbit.albums.milestones.list.invalidate({ albumId: activeAlbumId }); await utils.orbit.albums.list.invalidate(); setMilestoneTitle(""); setMilestoneNote(""); toast.success("Album milestone added."); }}><label>Milestone<input value={milestoneTitle} onChange={(event) => setMilestoneTitle(event.target.value)} maxLength={160} placeholder="The day we arrived" required /></label><label>Date<input type="date" value={milestoneDate} onChange={(event) => setMilestoneDate(event.target.value)} required /></label><label>Note <span>(optional)</span><textarea value={milestoneNote} onChange={(event) => setMilestoneNote(event.target.value)} maxLength={800} placeholder="What made this chapter matter?" /></label><button className="secondary-button" disabled={createMilestone.isPending}>{createMilestone.isPending ? <Loader2 className="spin" size={15} /> : "Add album milestone"}<Star size={14} /></button></form><div className="milestone-list">{milestones.data?.length ? milestones.data.map(({ milestone, authorName }) => <div className="milestone-row" key={milestone.id}><div><small>{formatDate(milestone.milestoneDate)} · {authorName ?? "You"}</small><b>{milestone.title}</b>{milestone.note && <p>{milestone.note}</p>}</div>{milestone.createdById === user?.id && <button className="quiet-delete" onClick={async () => { await removeMilestone.mutateAsync({ id: milestone.id }); await utils.orbit.albums.milestones.list.invalidate({ albumId: activeAlbumId }); await utils.orbit.albums.list.invalidate(); }}>Remove</button>}</div>) : <div className="empty-inline"><Star size={18} /><span>No milestones in this album yet.</span></div>}</div></div>}</GlassCard><GlassCard className="upload-card"><div><div className="eyebrow"><LockKeyhole size={14} /> PRIVATE MEDIA</div><h3>Add a shared moment</h3>
<p>Photos and videos are stored as private files; the database retains only their secure references and details. Either of us can choose a shared photo for the sphere.</p></div><form className="upload-form" onSubmit={upload}><label className={`drop-zone ${file ? "chosen" : ""}`}><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif,video/mp4,video/webm,video/quicktime,video/x-m4v" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /><ImagePlus size={21} /><span>{file ? file.name : "Choose photo or video"}</span><small>Choose from your phone gallery · up to 15 MB</small></label><label className="secondary-button camera-source"><input ref={cameraInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif" capture="environment" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /><Camera size={17} /> Take a photo</label><label>Caption<input value={caption} onChange={(e) => setCaption(e.target.value)} maxLength={500} placeholder="A tiny note to remember this by" /></label><label>Quote <span>(optional)</span><input value={quote} onChange={(e) => setQuote(e.target.value)} maxLength={280} placeholder="A line we want to carry with this memory" /></label>{quote.trim() && <blockquote className="quote-preview"><Quote size={15} />{quote.trim()}</blockquote>}<div className="soundtrack-fields"><label>Song title <span>(optional)</span><input value={songTitle} onChange={(event) => setSongTitle(event.target.value)} maxLength={160} placeholder="The song this memory sounds like" /></label><label>Artist <span>(optional)</span><input value={songArtist} onChange={(event) => setSongArtist(event.target.value)} maxLength={160} placeholder="Who made it?" /></label><label>Song link <span>(optional)</span><input type="url" value={songUrl} onChange={(event) => setSongUrl(event.target.value)} maxLength={1024} placeholder="https://..." /></label></div><label>Album <span>(optional)</span><select value={albumId ?? ""} onChange={(event) => setAlbumId(event.target.value ? Number(event.target.value) : null)}><option value="">No album</option>{albums.data?.map(({ album }) => <option key={album.id} value={album.id}>{album.name}</option>)}</select></label><label>Privacy<select value={visibility} onChange={(event) => setVisibility(event.target.value as typeof visibility)}><option value="pair">Visible to both of us</option><option value="private">Private to me</option></select></label><label>Date of this moment<input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label><button className="primary-button" disabled={create.isPending || prepareUpload.isPending}>{create.isPending || prepareUpload.isPending ? <Loader2 className="spin" size={17} /> : "Save private moment"}<Heart size={16} /></button></form></GlassCard>
    <div className="section-head"><div><div className="eyebrow">YOUR CONSTELLATION</div><h3>Shared memories</h3></div><span>{visibleMoments.length} shown · {stats.data?.moments ?? 0} total</span></div><div className="moment-tools"><label className="search-field"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search captions and dates" /></label><div className="filter-row" role="group" aria-label="Filter moments"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "photo" ? "selected" : ""} onClick={() => setFilter("photo")}>Photos</button><button className={filter === "video" ? "selected" : ""} onClick={() => setFilter("video")}>Videos</button><button className={favoritesOnly ? "selected" : ""} onClick={() => setFavoritesOnly((value) => !value)}><Star size={13} /> Favorites</button></div></div><div className="moment-grid">{moments.isLoading ? <LoadingScreen /> : visibleMoments.length ? visibleMoments.map(({ moment, authorName, reactions }) => <GlassCard key={moment.id} className="moment-card"><div className="media-frame">{moment.mediaType === "photo" ? <img src={moment.mediaUrl} alt={moment.caption || "Shared moment"} /> : <video src={moment.mediaUrl} controls preload="metadata" />}</div><div className="moment-meta"><small>{formatDate(moment.occurredAt)} · {authorName ?? "You"} · {moment.visibility === "private" ? "Private" : "For us"}</small><div className="memory-actions"><button className={moment.favorite ? "quiet-delete active-action" : "quiet-delete"} onClick={async () => { await toggleFavorite.mutateAsync({ id: moment.id }); await utils.orbit.moments.list.invalidate(); }}><Star size={13} fill={moment.favorite ? "currentColor" : "none"} /> {moment.favorite ? "Favorite" : "Save"}</button><button className={reactions?.some((reaction) => reaction.userId === user?.id && reaction.kind === "heart") ? "quiet-delete active-action" : "quiet-delete"} onClick={async () => { await react.mutateAsync({ id: moment.id, kind: "heart" }); await utils.orbit.moments.list.invalidate(); }}><Heart size={13} fill={reactions?.some((reaction) => reaction.kind === "heart") ? "currentColor" : "none"} /> {reactions?.filter((reaction) => reaction.kind === "heart").length ?? 0}</button><button className="quiet-delete" onClick={async () => { await react.mutateAsync({ id: moment.id, kind: "remember" }); await utils.orbit.moments.list.invalidate(); }}>Remember</button></div>{editingMoment?.id === moment.id ? <form className="inline-edit" onSubmit={async (event) => { event.preventDefault(); await update.mutateAsync({ id: moment.id, caption: editingMoment.caption || undefined, quote: editingMoment.quote || undefined, songTitle: editingMoment.songTitle || undefined, songArtist: editingMoment.songArtist || undefined, songUrl: editingMoment.songUrl || undefined, occurredAt: new Date(`${editingMoment.occurredAt}T12:00:00`) }); await utils.orbit.moments.list.invalidate(); setEditingMoment(null); toast.success("Your private moment was updated."); }}><label>Caption<input value={editingMoment.caption} maxLength={500} onChange={(event) => setEditingMoment({ ...editingMoment, caption: event.target.value })} /></label><label>Quote <span>(optional)</span><input value={editingMoment.quote} maxLength={280} onChange={(event) => setEditingMoment({ ...editingMoment, quote: event.target.value })} /></label><label>Song title <span>(optional)</span><input value={editingMoment.songTitle} maxLength={160} onChange={(event) => setEditingMoment({ ...editingMoment, songTitle: event.target.value })} /></label><label>Artist <span>(optional)</span><input value={editingMoment.songArtist} maxLength={160} onChange={(event) => setEditingMoment({ ...editingMoment, songArtist: event.target.value })} /></label><label>Song link <span>(optional)</span><input type="url" value={editingMoment.songUrl} maxLength={1024} onChange={(event) => setEditingMoment({ ...editingMoment, songUrl: event.target.value })} /></label><label>Date<input type="date" value={editingMoment.occurredAt} onChange={(event) => setEditingMoment({ ...editingMoment, occurredAt: event.target.value })} /></label><div className="record-actions"><button className="secondary-button" type="button" onClick={() => setEditingMoment(null)}>Cancel</button><button className="primary-button" disabled={update.isPending}>{update.isPending ? <Loader2 className="spin" size={14} /> : "Save changes"}</button></div></form> : <><p>{moment.caption || "A private moment"}</p>{moment.quote && <blockquote className="moment-quote"><Quote size={13} />{moment.quote}</blockquote>}{moment.songTitle && <a className="song-link" href={moment.songUrl || undefined} target="_blank" rel="noreferrer">♫ {moment.songTitle}{moment.songArtist ? ` · ${moment.songArtist}` : ""}</a>}{moment.createdById === user?.id && <div className="record-actions"><button className="quiet-delete" onClick={() => setEditingMoment({ id: moment.id, caption: moment.caption || "", quote: moment.quote || "", songTitle: moment.songTitle || "", songArtist: moment.songArtist || "", songUrl: moment.songUrl || "", occurredAt: new Date(moment.occurredAt).toISOString().slice(0, 10) })}>Edit</button><button className="quiet-delete" onClick={async () => { await update.mutateAsync({ id: moment.id, caption: moment.caption || undefined, occurredAt: new Date(moment.occurredAt), visibility: moment.visibility === "pair" ? "private" : "pair" }); await utils.orbit.moments.list.invalidate(); toast.success(moment.visibility === "pair" ? "This memory is private now." : "This memory is shared with us again."); }}>{moment.visibility === "pair" ? "Make private" : "Share with us"}</button><button className="quiet-delete" onClick={async () => { await remove.mutateAsync({ id: moment.id }); await utils.orbit.moments.list.invalidate(); await utils.orbit.relationship.get.invalidate(); }}>Remove</button></div>}</>}
            {moment.mediaType === "photo" && <div className="record-actions"><button className={relationship.data?.relationship?.featuredMomentId === moment.id ? "secondary-button cover-selected" : "secondary-button"} onClick={async () => { await setFeatured.mutateAsync({ id: moment.id }); await utils.orbit.relationship.get.invalidate(); toast.success("This shared photo is now featured for both of us."); }} disabled={setFeatured.isPending}>{relationship.data?.relationship?.featuredMomentId === moment.id ? "Featured memory" : "Feature for us"}</button></div>}</div></GlassCard>) : <div className="empty-state"><ImagePlus size={30} /><h3>{moments.data?.length ? "No matching private moments." : "Your constellation begins here."}</h3><p>{moments.data?.length ? "Try a different media filter." : "Add the first photo or video that holds meaning for us."}</p></div>}</div>
  </div>;
}
...
function SettingsPage() {
  ...
  const moments = trpc.orbit.moments.list.useQuery();
  const prepareUpload = trpc.orbit.moments.prepareUpload.useMutation();
  const createMoment = trpc.orbit.moments.create.useMutation();
  ...
  useEffect(() => { if (relationship.data?.relationship) { setOrbitName(relationship.data.relationship.displayName); setOrbitStartDate(new Date(relationship.data.relationship.startDate).toISOString().slice(0, 10)); setSelectedCoverId(relationship.data.relationship.coverMomentId ? String(relationship.data.relationship.coverMomentId) : ""); } }, [relationship.data?.relationship?.displayName, relationship.data?.relationship?.startDate, relationship.data?.relationship?.coverMomentId]);
  const sharedPhotos = moments.data?.filter(({ moment }) => moment.mediaType === "photo" && moment.visibility === "pair") ?? [];
  if (relationship.error || prefs.error || moments.error) return <DataError title="Your orbit settings could not be loaded." />;
  ...
  const saveCover = async () => { if (!selectedCoverId) return toast.error("Choose one of our shared photos first."); await setCover.mutateAsync({ id: Number(selectedCoverId) }); await utils.orbit.relationship.get.invalidate(); toast.success("Our sphere picture was updated."); };
  const uploadAndSetCover = async () => {
    if (!coverFile) return toast.error("Choose a photo for our sphere first.");
    const mimeType = detectMomentMimeType(coverFile);
    if (!mimeType || !mimeType.startsWith("image/")) return toast.error("Choose a JPEG, PNG, WebP, GIF, AVIF, HEIC, or HEIF photo.");
    if (coverFile.size > 15 * 1024 * 1024) return toast.error("Choose a photo smaller than 15 MB.");
    try {
      const prepared = await prepareUpload.mutateAsync({ filename: coverFile.name, mimeType });
      const uploadForm = new FormData();
      uploadForm.append("file", coverFile);
      Object.entries(prepared.uploadParams).forEach(([key, value]) => uploadForm.append(key, value));
      const uploadResponse = await fetch(prepared.uploadUrl, { method: "POST", body: uploadForm });
      if (!uploadResponse.ok) throw new Error(`Cloudinary upload failed (${uploadResponse.status}).`);
      const created = await createMoment.mutateAsync({ filename: coverFile.name, mimeType, fileKey: prepared.key, mediaUrl: prepared.url, caption: "Our sphere picture", visibility: "pair", fileSizeBytes: coverFile.size, occurredAt: new Date() });
      if (!created.id) throw new Error("The photo was uploaded but could not be selected for our sphere.");
      await setCover.mutateAsync({ id: created.id });
      await utils.orbit.moments.list.invalidate();
      await utils.orbit.relationship.get.invalidate();
      setCoverFile(null);
      if (coverInputRef.current) coverInputRef.current.value = "";
      toast.success("Our new sphere picture is now shared.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "The sphere picture could not be uploaded.");
    }
  };
  const isOwner = relationship.data?.member?.role === "owner";
  return <div className="page-grid settings-grid"><GlassCard><div className="eyebrow"><LockKeyhole size={14} /> LINKED PAIR ACCESS</div><h3>Invite your one partner</h3><p>{relationship.data?.partner ? `${relationship.data.partner.name ?? "Your partner"} is already linked. This orbit cannot add a third person.` : "Create a one-time invitation. Only the first authenticated person to accept it can join this orbit."}</p>{isOwner && !relationship.data?.partner && <><button className="primary-button" onClick={createInvite} disabled={invite.isPending}>{invite.isPending ? <Loader2 className="spin" size={17} /> : "Create private invitation"}<Copy size={16} /></button>{inviteLink && <div className="invite-link"><input readOnly value={inviteLink} /><button onClick={() => navigator.clipboard?.writeText(inviteLink)} aria-label="Copy invitation"><Copy size={16} /></button></div>}</>}</GlassCard>{isOwner && <GlassCard><div className="eyebrow"><Settings size={14} /> ORBIT OWNER CONTROLS</div><h3>Update shared details</h3><p>As the orbit owner, you can change the shared name and relationship start date. Personal records remain editable only by the person who created them.</p><form className="form-stack" onSubmit={saveOrbitSettings}><label>Orbit name<input value={orbitName} onChange={(event) => setOrbitName(event.target.value)} minLength={2} maxLength={80} required /></label><label>Relationship start date<input type="date" value={orbitStartDate} onChange={(event) => setOrbitStartDate(event.target.value)} required /></label><button className="secondary-button" disabled={updateRelationship.isPending}>{updateRelationship.isPending ? <Loader2 className="spin" size={15} /> : "Save shared details"}</button></form></GlassCard>}<GlassCard className="sphere-settings-card"><div className="eyebrow"><ImagePlus size={14} /> SPHERE PHOTO</div><h3>Choose our picture</h3><p>Either of us can change the shared sphere picture whenever we want. Only photos marked “Visible to both of us” appear here.</p><label className={`drop-zone ${coverFile ? "chosen" : ""} sphere-upload-picker`}><input ref={coverInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif" onChange={(event) => setCoverFile(event.target.files?.[0] ?? null)} /><ImagePlus size={18} /><span>{coverFile ? coverFile.name : "Upload a new sphere photo"}</span><small>It will be added as a shared memory and selected for our sphere.</small></label><button type="button" className="primary-button" onClick={uploadAndSetCover} disabled={prepareUpload.isPending || createMoment.isPending || setCover.isPending || !coverFile}>{prepareUpload.isPending || createMoment.isPending || setCover.isPending ? <Loader2 className="spin" size={15} /> : "Upload and use in our sphere"}<ImagePlus size={15} /></button>{sharedPhotos.length > 0 && <><div className="settings-divider" /><label>Or choose an existing shared photo<select value={selectedCoverId} onChange={(event) => setSelectedCoverId(event.target.value)}><option value="">Choose a shared photo</option>{sharedPhotos.map(({ moment }) => <option key={moment.id} value={moment.id}>{moment.caption || `Photo from ${formatDate(moment.occurredAt)}`}</option>)}</select></label><button type="button" className="secondary-button" onClick={saveCover} disabled={setCover.isPending || !selectedCoverId}>{setCover.isPending ? <Loader2 className="spin" size={15} /> : "Use selected photo in our sphere"}<ImagePlus size={15} /></button></>}</GlassCard><GlassCard><div className="eyebrow"><Bell size={14} /> NOTIFICATION CONSENT</div><h3>Choose what reaches you</h3><p>Notifications appear inside the private orbit. Wellness and reminders begin off so sensitive details stay quiet unless you opt in.</p><div className="preference-list">{([ ["memoriesEnabled", "New memories"], ["feelingsEnabled", "Shared feelings"], ["wellnessEnabled", "Shared wellness entries"], ["remindersEnabled", "My private reminders"] ] as const).map(([key, label]) => <label className="switch-row" key={key}><span>{label}</span><input type="checkbox" checked={form[key]} onChange={(e) => setForm((current) => ({ ...current, [key]: e.target.checked }))} /><i /></label>)}</div><button className="secondary-button" onClick={async () => { await updatePrefs.mutateAsync(form); toast.success("Notification choices updated."); }} disabled={updatePrefs.isPending}>Save choices</button></GlassCard></div>;
}
